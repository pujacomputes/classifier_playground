...
class DistortImageFolder(data.Dataset):
    def __init__(self, root, method, severity,env, transform=None, target_transform=None,
                 loader=default_loader):
        classes, class_to_idx = find_classes(root)
        imgs = load_dataset_domainnet(root)
        if len(imgs) == 0:
    ...
    def __getitem__(self, index):
        ...
        save_path = '/usr/workspace/wsa/trivedi1/vision_data/DomainNet-Corrupted/{env}/{method}/{severity}/{class_idx}'.format(env=self.env,
            method = self.method.__name__,
            severity= self.severity,
            class_idx = self.idx_to_class[target] )

        # save_path = '/p/lustre3/trivedi1/vision_data/PACS_Calibration/{env}/{method}/{severity}/{class_idx}'.format(env=self.env,
        #     method = self.method.__name__,
        #     severity= self.severity,
        #     class_idx = self.idx_to_class[target] )
...
class DomainNet(data.Dataset):
    def __init__(self, domain, method, severity, split='train', root=ROOT,
        ...
        self.data = load_dataset_domainnet(domain_list, split, version)
        if verbose:
            print ('\t=> Domains: ', domain_list, "--  split: ",split)
            print('\t=> Num images: ',len(self.data))
            print('\t=> Num classes: ', self.get_num_classes())
            print("\t=> Transform: ",self._transform)
            print("\t=> Method: ",self.method)
            print("\t=> Severity: ",self.method)
            img_path,_ = self.data[0]
            save_path = '/usr/workspace/wsa/trivedi1/vision_data/DomainNet-Corrupted/{method}/{severity}/{img_path}'.format(method = self.method.__name__,
                severity= self.severity, img_path=img_path)
            print("\t => Sample Save Path: ",save_path)
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        """
        After transforming with the corruption,
        we need to save the file according to the corruption and severity.
        """
        save_path = '/usr/workspace/wsa/trivedi1/vision_data/DomainNet-Corrupted/{method}/{severity}/{img_path}'.format(method = self.method.__name__,
            severity= self.severity,
            img_path = path)
...
# def save_distorted(method=gaussian_noise,env='photo'):
...
#             root="/usr/workspace/wsa/trivedi1/vision_data/DomainNet/{env}/".format(env=env),
#             method=method, severity=severity,
#             transform=trn.Resize((224,224)),
#             env=env)
...
def main():
    ...
    for method_name in d.keys():
        for severity in range(1,6):
            for c in classes:
                save_path = '/usr/workspace/wsa/trivedi1/vision_data/DomainNet-Corrupted/{method}/{severity}/{env}/{class_name}'.format(env=args.env,
    ...
    for method_name in args.corruptions:
        ...
        save_distorted(method=d[method_name],env=args.env)
