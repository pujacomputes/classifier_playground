...
from utils import get_corrupted_loader, get_dataloaders, get_oodloader,get_transform, NUM_CLASSES_DICT,norm_dict, get_calibration_loader
    ...
    if cal_dataset in ['id-c','id-c-bar']:
        ...
        for corruption in corrs:
            for sev in range(1,6): 
                confidence = []
                correct = []
                test_loader = get_calibration_loader(args,
                    cal_dataset,
...
def test_cal(net, cal_dataset, args,clean_test_dataset=None):
    ...
    if cal_dataset in ['id-c','id-c-bar']:
        ...
        for corruption in corrs:
            ...
            test_loader = get_calibration_loader(args,cal_dataset, corruption=corruption,clean_test_dataset=clean_test_dataset)
            num_correct = 0
            with torch.no_grad():
                for batch_idx, (data, target) in enumerate(test_loader):
                    data, target = data.cuda(), target.cuda()
...
def arg_parser_eval():
    ...
    parser.add_argument('--cal_datasets', nargs='*', help='Datasets on Evaluate Calibration (ID/ODD)', required=False)
    parser.add_argument('--corr_datasets', nargs='*', help='CBAR vs. C; Indist/OODist', required=False)
...
def main():
    ...
    if args.eval_all or args.eval_A:
        adversary = PGD(epsilon=2./255, num_steps=10, step_size=0.5/255).cuda()
    ...
    if args.eval_all or args.eval_C:
        if 'domain' in args.dataset: 
            ...
            _, clean_test_dataset_c = get_dataloaders(args=args,
                train_aug="test",
                test_aug="test",
                train_transform=train_transform,
                test_transform=test_transform,
    ...
    if args.eval_all or args.eval_Cal:
        ...
        if "domain" in args.dataset:
            ...
            for calib_data in args.cal_datasets:
                ...
                print("="*30)
                calib_acc, calib_err = test_cal_domainnet(net, calib_data, args) 
                print("=> Living17, {1}, Calib. Err.: {0:.4f}".format(calib_err,calib_data))
                with open("{}/living17-calibration-sketch-v2.csv".format(safety_logs_prefix),"a") as f:
                    write_str = "{save_name},{cal_data},{calib:.4f}\n".format(save_name=save_name,cal_data=calib_data, calib=calib_err)
                    f.write(write_str)
                with open("{}/living17-cal-acc.csv".format(safety_logs_prefix),"a") as f:
                    write_str = "{save_name},{cal_data},{acc:.4f}\n".format(save_name=save_name,acc=calib_acc,cal_data=calib_data)
                    f.write(write_str)
        elif "cifar" in args.dataset:
            _, clean_calib_dataset = get_dataloaders(args=args,
                train_aug="test",
                test_aug="test",
                train_transform=train_transform,
                test_transform=test_transform,
            ...
            for calib_data in args.cal_datasets:
                ...
                print("=> Calib Dataset: ",calib_data)
                print("="*30)
                calib_acc, calib_err = test_cal(net, calib_data, args,clean_test_dataset=clean_calib_dataset)
                with open("{}/cifar-calibration.csv".format(safety_logs_prefix),"a") as f:
                    write_str = "{save_name},{cal_data},{calib:.4f}\n".format(save_name=save_name,calib=calib_err,cal_data=calib_data)
                    f.write(write_str)
