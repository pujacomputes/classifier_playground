    ...
    with torch.no_grad():
        for data, targets in tqdm.tqdm(loader,disable=False):
            ...
            if "clip" in args.arch:
                ...
                features.append(reps.detach().cpu().numpy())
                labels.append(targets.detach().cpu().numpy())
            #See: https://github.com/rwightman/pytorch-image-models/blob/4714a4910e111c5b39afa834975ecc257f34467a/timm/models/vision_transformer.py#L531
            if "vit" in args.arch:
                #using a timm model. called the
...
def linear_probe_soup(args, net, train_loader,test_loader, train_aug, train_transform):
    ...
    for epochs in range(args.epochs):
        ...
        print(
            "SOUP, Epoch: {0} -- Loss: {1:.3f} -- Train Error: {2:.4f} -- Test Error: {3:.4f} ".format(
                epochs,
                loss_avg,
                100 - 100.0 * train_acc,
                100 - 100.0 * test_acc,
...
def main():
    ...
    if args.protocol in ['lp','lp+ft','vatlp+ft','vatlp','fgsmlp','fgsmlp+ft','udplp','udplp+ft','souplp','souplp+ft']:
        ...
        """
        if args.resume_lp_ckpt.lower() != "none" and args.protocol in ['lp+ft','vatlp+ft','fgsm+ft','udplp+ft','souplp+ft']:
            print("****************************")
            ...
            if "vat" in args.protocol and args.resume_lp_ckpt.lower() == "none" :
            ...
            elif args.protocol in ['souplp', 'souplp+ft'] and args.resume_lp_ckpt.lower() == "none" :
                print("=#"*20)
                ...
                print("=*"*60)
                use_head = True if "vit" in args.arch else False
                print("USE HEAD: ",use_head)
                net = freeze_layers_for_lp(net,use_head=use_head)
                if "vit" in args.arch:
                    print("USING VIT PARAMS")
                    optimizer = torch.optim.SGD(
    ...
    """
    if args.protocol in ['lp+ft','ft','lpfrz+ft','vatlp+ft','fgsmlp+ft','udplp+ft','souplp+ft']:
        ...
        """
        if args.protocol in ['lp+ft','vatlp+ft','fgsmlp+ft','udplp+ft','souplp+ft'] and args.resume_lp_ckpt.lower() == 'none':
