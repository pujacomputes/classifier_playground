    ...
    with torch.no_grad():
        for data, targets in tqdm.tqdm(loader):
            ...
            if args.arch == 'resnet50':
                ...
                reps = torch.nn.functional.adaptive_avg_pool2d(model.forward_features(data),1)
...
#     best_intercept, 
#     best_c, 
#     best_i) = test_log_reg_warm_starting(train_features=normalized_train_features, 
#         train_labels=train_labels,
#         val_features=normalized_test_features, 
#         val_labels=val_labels, num_cs=100, start_c=-7, end_c=2, max_iter=200, random_state=0)
#     print("Best C: {0} Best i: {}".format(best_c,best_i)) 
...
def linear_probe_vat(args, net, train_loader, val_loader, test_loader, train_aug, train_transform):
    net.eval()
    #TODO: pu the trainloader back!
    if train_aug != 'test':
        print("*"*50)
        print("CAUTION: Train Aug should be set to test for VAT!")
        print("*"*50)
    train_features, train_labels = extract_features(args,model=net, loader=train_loader,train_aug=train_aug,train_transform=train_transform)
    val_features, val_labels = extract_features(args,model=net, loader=val_loader,train_aug=train_aug,train_transform=train_transform)
    #test_features, test_labels = extract_features(args,model=net, loader=test_loader,train_aug='test',train_transform=None)
    print(train_features.shape)

    rep_train_dataset = torch.utils.data.TensorDataset(torch.Tensor(train_features),torch.Tensor(train_labels).long())
    rep_train_dataloader = torch.utils.data.DataLoader(rep_train_dataset,batch_size=512,shuffle=True) 
    
    rep_val_dataset = torch.utils.data.TensorDataset(torch.Tensor(val_features),torch.Tensor(val_labels).long())
    rep_val_dataloader = torch.utils.data.DataLoader(rep_val_dataset,batch_size=512,shuffle=True) 
    """
    ...
    for epochs in range(args.epochs):
        ...
        for batch_idx, (data, target) in enumerate(rep_train_dataloader):
            ...
            loss = consistency_loss + 0.01 * lds
            loss.backward()
            optimizer.step()
            scheduler.step()
        ...
        _,val_acc = test(fc, rep_val_dataloader) 
        vat_avg /= len(rep_train_dataloader) 
        consistency_loss_avg /= len(rep_train_dataloader) 
        loss_avg /= len(rep_train_dataloader) 
        print("Epoch: {4} -- VAT: {0:.4f} -- Con: {1:.4f} -- Tot.:{2:.4f} -- Train Acc: {3:.4f} -- Val Acc: {5:.4f}".format(vat_avg,consistency_loss_avg,loss_avg,train_acc,epochs,val_acc))
    #Set the classifier weights.s
    if args.arch == 'resnet50':
        net.fc = fc 
    else:
...
def main():
    ...
    if args.protocol in ['vatlp','vatlp+ft']:
        ...
        log_path = os.path.join("/p/lustre1/trivedi1/compnets/classifier_playground/logs",
                            "sklp+" + save_name + '_training_log.csv')
        ...
        train_loader, val_loader, test_loader = get_dataloaders(args=args, 
            train_aug=args.train_aug,
            test_aug=args.test_aug, 
            train_transform=train_transform,
            test_transform=test_transform,
            return_valoader=True) 
        ...
        """
        if args.resume_lp_ckpt.lower() != "none" and args.protocol == 'sklp+ft':
            print("****************************")
            print("Loading Saved SKLP Ckpt")
            ...
            net, ckpt = linear_probe_vat(args, net, train_loader, val_loader, test_loader, args.train_aug, train_transform)
            save_name = "sklp+"+save_name

            """
            Save LP Final Ckpt.
            """
            s = "sklp+{save_name}_final_checkpoint_{epoch:03d}_pth.tar".format(save_name=save_name,epoch=args.epochs)
